...
#Y_j=Y[,j]
#weights are sampling weights
target_function_ab=function(ab, x, Y_j, weights, z,  mu_pres, sigma_pres, q){
  N=dim(x)[1]
  p=dim(x)[2]
  n=length(z)
  a_j=ab[1]
...
grad_phi_ab=function(ab, x, Y_j, weights, z, mu_pres, sigma_pres, q){
  N=dim(x)[1]
  p=dim(x)[2]
  n=length(z)
  a_j=ab[1]
...
mml=function(a, b,  x, dat, weights, z, mu, sigma, w){ #q is quadrature
  N=dim(x)[1]
  p=dim(x)[2]
  n=length(z)
  J=dim(dat)[2]
...
EM_2PL_inference <- function(a, b, g, x, dat, weights, z, mu, sigma, w, ite, tol = 0.001){
  ...
  while(max(c(abs(phi0-phi1)), abs(mu0-mu1), abs(sigma0-sigma1))>tol){#max
    ...
    for(j in 1:J){#update phi_j one by one
      if(j==1){
        par_updates_ab = optim(par=phi1[j, 2], fn=target_function1, gr=grad_phi1, method = "L-BFGS-B", x=x, Y_j=dat[,j], weights=weights, z=z, mu_pres=mu0,  sigma_pres=sigma0, q=quadrature, lower=-5, upper=5)
        phi1[j, 2]=par_updates_ab$par
      }
      else{
        for (l in 1:ite){
          par_updates_ab = optim(par=phi1[j, c(1,2)], fn=target_function_ab, gr=grad_phi_ab, method = "L-BFGS-B", x=x, Y_j=dat[,j], weights=weights, z=z, mu_pres=mu0,  sigma_pres=sigma0, q=quadrature, lower=-5, upper=5)
    ...
    MLL = mml(a=phi1[,1], b=phi1[,2],  x, dat, weights, z, mu=mu1, sigma=sigma1, w)
    print(MLL) 
  }
  list(mu=mu1, sigma=sigma1, alpha.vec=phi1[, 1], beta.vec=phi1[, 2], post=quadrature);
}
...
#g_ini= read.csv('gamma_math_2022_1000EM.csv', header=F)
#a_ini= read.csv('alpha_math_2022_1000EM.csv', header=F)[,1]
#b_ini= read.csv('beta_math_2022_1000EM.csv', header=F)[,1]
#mu_ini= read.csv('mu_math_2022_1000EM.csv', header=F)[,1]
#sigma_ini= read.csv('sigma_math_2022_1000EM.csv', header=F)[,1]
...
#write.table(g_hat, sep=",",  col.names=FALSE, row.names=FALSE, file = 'gamma_math_2022_baseline.csv')
write.table(a_hat, sep=",",  col.names=FALSE, row.names=FALSE, file = 'alpha_reading_2022_baseline.csv')
write.table(b_hat, sep=",",  col.names=FALSE, row.names=FALSE, file = 'beta_reading_2022_baseline.csv')
write.table(mu_hat, sep=",",  col.names=FALSE, row.names=FALSE, file = 'mu_reading_2022_baseline.csv')
write.table(sigma_hat, sep=",",  col.names=FALSE, row.names=FALSE, file = 'sigma_reading_2022_baseline.csv')
write.table(post_hat, sep=",",  col.names=FALSE, row.names=FALSE, file = 'posterior_reading_2022_baseline.csv')
