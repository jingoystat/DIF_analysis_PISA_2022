...
#Evaluate target function at anchor item, phi_j=(a_j, b_j, g_j1,...,g_jp), i.e. negative log-likelihood at phi_j
#Y_j=Y[,j]
target_function1=function(phi_j, x, Y_j, z,  mu_pres, sigma_pres, q){
  ...
  #Evaluate parameter dependent part of the log-likelihood
  Theta=tran(z, x, mu_pres, sigma_pres)
  temp=Theta*a_j+b_j*(rep(1,N)%*%t(rep(1, n)))+(x%*%g_j)%*%t(rep(1, n)) #N by n
  t2=Y_j%*%t(rep(1, n))*temp-log(1+exp(temp)) #The parameter dependent term of log-likelihood, N by n
  res=-sum(q*t2, na.rm = TRUE)
  return(res/N)
}

target_function1_b=function(phi_j, x, Y_j, z,  mu_pres, sigma_pres, q){
  N=dim(x)[1]
  p=dim(x)[2]
  n=length(z)
  a_j=1
...
target_function1_g=function(ab, k, gk, gp, x_k, Y_k, z, ind_k, mu_pres, sigma_pres, q){
  N_k=dim(x_k)[1]
  p=dim(x_k)[2]
  n=length(z)
  a_j=ab[1]
...
#Evaluate gradient with respect to phi=(a, b) in anchor set
...
grad_phi1_b=function(phi_j, x, Y_j, z, mu_pres, sigma_pres, q){
  N=dim(x)[1]
  p=dim(x)[2]
  n=length(z)
  a_j=1
...
grad_phi1_g=function(ab, k, gk, gp, x_k, Y_k, z, ind_k,  mu_pres, sigma_pres, q){
  N_k=dim(x_k)[1]
  p=dim(x_k)[2]
  n=length(z)
  a_j=ab[1]
...
EM_2PL_inference_lrt <- function(a, b, g, x, dat, z, mu, sigma, w, ite, RMSD_mtx, RMSD_threshold, tol = 0.001){
  ...
  while(max(c(abs(phi0-phi1)), abs(mu0-mu1), abs(sigma0-sigma1), na.rm=T)>tol){#max
    #MLL0=MLL
    phi0=phi1
    mu0=mu1
    sigma0=sigma1
    ...
    for (k in 1:p){
      par_updates = optim(par=sigma0[k], fn=target_var, gr=grad_var, method = "L-BFGS-B", k=k,x=x, z=z, mu_pres=mu0,  sigma_pres=sigma0, q=quadrature, lower=0.05, upper=5)
...
information=function(a, b, g, x, dat, z, mu, sigma, q, RMSD_mtx, RMSD_threshold){
  J=length(a)
  n=dim(q)[2]
  N=dim(dat)[1]
  p=dim(x)[2]
  ...
  #anchor_idx=seq(from=1, by=p, length.out=length(anchor))
  #anchor_idx2=seq(from=2, by=p, length.out=length(anchor))
  #anchor_idx = c(anchor_idx1, anchor_idx2)
  res=res[ , -c(a1_ind, (2*p+2*J+anchor_ind))]
  res=res[-c(a1_ind, (2*p+2*J+anchor_ind)), ]
...
coverage_proposed_inference=function(N, sig, gamma, beta.vec,   alpha.vec, mu, sigma, x, ite, RMSD_threshold, sig_list, seed){
  set.seed(seed)
  J=length(alpha.vec)
  p=dim(x)[2]
  ...
  for (l in 1:1){
    ...
    g[RMSD_mtx < RMSD_threshold]
    
    #Train model
    r=EM_2PL_inference_lrt(a=alpha.vec,  b=beta.vec, g=gamma, x=x, dat=dat, z, mu=mu, sigma=sigma, w, ite=ite, RMSD_mtx, RMSD_threshold, tol = 1)
    g_hat=r$gamma.vec
    mu_hat=r$mu
    sigma_hat=r$sigma
    a_hat=r$alpha.vec
    ...
    cor = c(cor, cor(as.numeric(mu_hat), mu.vec, method = "kendall"))
    
    info=information(a=a_hat, b=b_hat, g=g_hat, x=x, dat, z, mu=mu_hat, sigma=sigma_hat, q=r$post, RMSD_mtx, RMSD_threshold)
  ...
  return(list(alpha=alpha, beta=beta, gamma=rgamma, mu=r_mu, sigma=r_sigma, RMSD_mtx_ls=RMSD_mtx_ls, anchor_ls=anchor_ls, cor=cor))
}
